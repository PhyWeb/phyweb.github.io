...
export default class Calculation {
  ...
  createConfiguredMathInstance() {
    ...
    // Redéfinir l'opérateur de multiplication * pour être élément par élément
    const originalMultiply = mathInstance.multiply;
    const typedMultiply = mathInstance.typed('multiply', {
        'Array | Matrix, Array | Matrix': function (a, b) {
            return mathInstance.dotMultiply(a, b);
        },
        // Fallback pour les autres cas (matrice * scalaire, etc.) qui fonctionnent déjà correctement
        'any, any': function (a, b) {
            return originalMultiply(a, b);
        }
    });
    mathInstance.import({ multiply: typedMultiply }, { override: true });
  ...
  evaluateBlock(formulas, initialScope) {
    ...
    const scope = {};
    // On s'assure que le scope initial ne contient que des Array
    for (const key in initialScope) {
        scope[key] = toArr(initialScope[key]);
    }
